...
function curl_wrap() {
	curl --request GET --header "$AUTH_HEADER" "$@"
}
...
address="https://api.soundcloud.com/users/$username/favorites?limit=200&linked_partitioning=1"
j=0
lastcount=0
echo "done"
...
for (( i=0; i<number_of_likes; i+=200 )); do
	list=$(curl_wrap "$address" 2> /dev/null)
...
for (( i=0; i<number_of_likes; i+=200 )); do
	...
	for (( j=0; j<listcount; j++ )); do
		...
		if [[ "${downloadable[$j]}" == false ]]; then
			download_url="$(curl_wrap "${stream_url[$j]}" 2> /dev/null | jq -r '.location')"
		else
			download_url="$(echo $list|jq -r ".collection[$j].download_url")"
			fileformat="$(curl_wrap -I -L "$download_url" 2> /dev/null)"
		...
		if [ "$DEBUG" -eq 0 ]; then
			...
			curl_wrap -L -o "$filename" "$download_url" &> /dev/null
			echo -n "."
			...
			curl_wrap -o artwork.jpg ${thumbnail_url[$j]} &> /dev/null
